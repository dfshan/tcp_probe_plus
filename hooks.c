...
#endif
{
    ...
    list_for_each_entry_safe(flow, temp, &tcp_flow_list, list) {
        ...
        struct timespec64 tv = ktime_to_timespec64(ktime_sub(tstamp, flow->tstamp));
...
write_flow(int type, struct tcp_hash_flow *tcp_flow, struct tcp_tuple *tuple, ktime_t tstamp,
        const struct sock *sk, const struct sk_buff *skb, u8 tcp_flags, u16 length,
        u32 seq_num, u32 ack_num, long reserved)
{
    ...
    if (tcp_probe_avail() > 1) {
        ...
        p->seq_num = seq_num;
        p->ack_num = ack_num;
        if (tcp_flow->cong_ops != NULL) {
            tcp_flow->cong_ops->read(sk, p->cong_data);
            p->sprint_cong_data = tcp_flow->cong_ops->sprint;
        } else {
            p->sprint_cong_data = NULL;
        }
        tcp_probe.head = (tcp_probe.head + 1) & (bufsize - 1);
    } else {
        TCPPROBE_STAT_INC(ack_drop_ring_full);
...
void (*real_tcp_done)(struct sock *sk);
/*
...
void (*real_tcp_rcv_established)(struct sock *sk, struct sk_buff *skb,
                ...
                const struct tcphdr *th, unsigned int len)
{
    ...
    if ((port == 0 || ntohs(tuple.dport) == port ||
        ...
        if (!tcp_flow) {
            if (maxflows > 0 && atomic_read(&flow_count) >= maxflows) {
                ...
                set_cong_ops(tcp_flow, inet_csk(sk)->icsk_ca_ops->name);
                should_write_flow = 1;
            }
        } else {
        /* if the difference between timestamps is >= probetime then write the flow to ring */
            struct timespec64 tv = ktime_to_timespec64(ktime_sub(tstamp, tcp_flow->tstamp));
    ...
    return real_tcp_rcv_established(sk, skb, th, len);
}

int (*real_tcp_transmit_skb)(struct sock *sk, struct sk_buff *skb, int clone_it,
                gfp_t gfp_mask, u32 rcv_nxt);
/*
* Hook inserted to be called before each sent packet.
* Note: arguments must match tcp_transmit_skb()!
*/
int fh_tcp_transmit_skb(struct sock *sk, struct sk_buff *skb, int clone_it,
                gfp_t gfp_mask, u32 rcv_nxt)
{
    const struct tcp_sock *tp = tcp_sk(sk);
    const struct inet_sock *inet = inet_sk(sk);
...
skip:
    return real_tcp_transmit_skb(sk, skb, clone_it, gfp_mask, rcv_nxt);
}

void (*real_tcp_retransmit_timer)(struct sock *sk);
/*
...
skip:
    return real_tcp_retransmit_timer(sk);
}


struct sock *(*real_tcp_v4_syn_recv_sock)(
...
struct sock *fh_tcp_v4_syn_recv_sock(
    ...
    return real_tcp_v4_syn_recv_sock(
        sk, skb, req, dst,
        req_unhash, own_req
    );
}
...
int (*real_tcp_v4_do_rcv)(struct sock *sk, struct sk_buff *skb);
/*
* Hook inserted to be called before each receive packet.
...
int fh_tcp_v4_do_rcv(struct sock *sk, struct sk_buff *skb)
{
    ...
    return real_tcp_v4_do_rcv(sk, skb);
}
