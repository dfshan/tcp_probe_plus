...
static int tcpprobe_open(struct inode * inode, struct file * file)
{
    ...
    tcp_probe.start_datetime = timespec64_to_ktime(ts);
...
static int tcpprobe_sprint(char *tbuf, int n)
{
    ...
    if (p->sprint_cong_data != NULL) {
        copied += p->sprint_cong_data(tbuf+copied, n-copied, p->cong_data);
    }
#if GET_USER_AGENT != 0
    if (p->user_agent[0] != '\0') {
...
const struct proc_ops tcpprobe_fops = {
    .proc_open     = tcpprobe_open,
    .proc_read    = tcpprobe_read,
    .proc_lseek  = noop_llseek,
};

const struct proc_ops tcpprobe_stat_fops = {
    .proc_open  = tcpprobe_seq_open,
    .proc_read  = seq_read,
