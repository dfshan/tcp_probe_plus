...
static struct ftrace_hook tcp_hooks[] = {
    HOOK("__tcp_transmit_skb",  fh_tcp_transmit_skb,  &real_tcp_transmit_skb),
    HOOK("tcp_v4_do_rcv",  fh_tcp_v4_do_rcv,  &real_tcp_v4_do_rcv),
    HOOK("tcp_done",  fh_tcp_done,  &real_tcp_done),
    HOOK("tcp_retransmit_timer",  fh_tcp_retransmit_timer,  &real_tcp_retransmit_timer),
    HOOK("tcp_v4_syn_recv_sock",  fh_tcp_v4_syn_recv_sock,  &real_tcp_v4_syn_recv_sock),
};
...
static __init int tcpprobe_init(void)
{
    ...
    struct timespec64 ct_ts;
    struct rtc_time ct_tm;
    unsigned long totalpages = 0;

#if LINUX_VERSION_CODE < KERNEL_VERSION(5,0,0)
    totalpages = totalram_pages;
#else
    totalpages = totalram_pages();
#endif
    ...
    if (!hashsize) {
        hashsize = (((totalpages << PAGE_SHIFT) / 16384)
                        / sizeof(struct hlist_head));
        if (totalpages > (1024 * 1024 * 1024 / PAGE_SIZE)) {
            hashsize = 16384;
        }
    }
    if (hashsize < 32) {
    ...
    if (!proc_create(PROC_TCPPROBE, S_IRUGO, INIT_NET(proc_net), &tcpprobe_fops)) {
...
#endif
*/
    ret = fh_install_hooks(tcp_hooks, ARRAY_SIZE(tcp_hooks));
    if (ret) {
        pr_err("Unable to install ftrace hooks");
        goto err1;
    }

    do_settimeofday64(&ct_ts);
    start_time = timespec64_to_ktime(ct_ts);

    /*UTC +8 time*/
    ct_ts.tv_sec += 28800;
    rtc_time64_to_tm((unsigned long) ct_ts.tv_sec, &ct_tm);
...
static __exit void tcpprobe_exit(void)
{
    ...
    pr_info("Start exiting tcp probe.\n");
